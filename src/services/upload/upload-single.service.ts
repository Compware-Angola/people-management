...
export async function uploadSingleFile(
  ...
  return uploadApi('upload-s3/single', {
    method: 'POST',
    body: formData,
  }).json<ResponseUpload>()
}
...
export async function uploadMultipleFiles(
  ...
  return uploadApi('upload-s3/multiple', {
    method: 'POST',
    body: formData,
  }).json<ResponseUploadMultiple>()
}
...
export async function getFileUrl(key: string, expiry?: number) {
  ...
  return uploadApi(`upload-s3/view?${params.toString()}`, {
    method: 'GET',
  }).json<ResponseView>()
}

export async function deleteFile(key: string) {
  return uploadApi('upload-s3', {
    method: 'DELETE',
